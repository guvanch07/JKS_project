  ...
  group('getRegistration', () {
    test(
      ...
      () async {
        ...
        expect(
          await repository.getJobs(mockName),
          isA<ApiAuthorizationResponse>(),
        );
      },
    );
    //! report of test: test is faild the reason is inctance of Job isEmpty dart core
    //todo fix and test again
    test(
      'returns with error',
      () async {
        final cancelToken = MockCancelToken();
        ...
        expect(
          () async => await repository.getJobs(mockName),
          throwsA(error),
        );
      },
    );
